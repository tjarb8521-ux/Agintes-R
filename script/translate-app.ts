...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "agintes/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".agintes/glossary/zh-cn.md"
  if (locale === "zht") return ".agintes/glossary/zh-tw.md"
  return `.agintes/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Agintes did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Agintes returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Agintes session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Agintes runs for 'all' (default: 4)
      --model <provider/id>  Agintes model (default: agintes/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Agintes
...
`)
  ...
  if (failed.length) console.error(`\nAgintes failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  const exported = Bun.spawn(["agintes", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["agintes", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
