...
import { createOpencode } from "@agintes-ai/sdk"
...
`)
  ...
  const agintes = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await agintes.client.session.create()
    const result = await agintes.client.session
      .prompt({
