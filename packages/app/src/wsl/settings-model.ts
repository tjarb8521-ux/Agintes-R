...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-agintes" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpencodeAction(check?: WslOpencodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Agintes"
  if (check.matchesDesktop === false) return "Update Agintes"
}
...
export function addServerViewModel(input: {
  ...
  const agintesCheck = selectedDistro ? (state?.agintesChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.agintesChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.agintesMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  agintesCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-agintes" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.agintesCheck)) {
    const update = !!input.agintesCheck?.resolvedPath && input.agintesCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.agintesChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.agintesChecks[item.name]) return [`agintes:${item.name}`]
    return []
  })
