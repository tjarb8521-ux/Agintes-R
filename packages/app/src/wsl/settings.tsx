    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.agintesChecks[item.config.distro]
          const agintesAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-agintes" && wsl.data.job.distro === item.config.distro
          return (
