...
import { SessionStatusEvent } from "@agintes-ai/schema/session-status-event"
import { SessionV1 } from "@agintes-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Agintes/TimelineStability"
...
export const model = { providerID: "agintes", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("agintes.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["agintes"],
    default: { providerID: "agintes", modelID: "claude-opus-4-6" },
  }
}
