...
import { currentSession, mockAgintesServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Agintes/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#agintes-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "agintes", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("agintes.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
