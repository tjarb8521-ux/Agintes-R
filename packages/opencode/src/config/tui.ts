...
import { isRecord } from "@agintes-ai/tui/util/record"
...
import { TuiKeybind } from "@agintes-ai/tui/config/keybind"
...
import { TuiConfig } from "@agintes-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@agintes/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old agintes.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.agintes`
  ...
  // 4. `.agintes` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".agintes") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".agintes") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
