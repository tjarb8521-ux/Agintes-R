...
const TUI_SCHEMA_URL = "https://agintes.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from agintes.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const agintes = await agintesFiles(input)
  for (const file of agintes) {
...
async function agintesFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "agintes"),
    ...(await Filesystem.findUp(["agintes.json", "agintes.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "agintes"))
  }
