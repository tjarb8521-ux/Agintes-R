...
const MANAGED_PLIST_DOMAIN = "ai.agintes.managed"

// Keys injected by macOS/MDM into the managed plist that are not Agintes config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/agintes"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "agintes")
    default:
      return "/etc/agintes"
  }
}
