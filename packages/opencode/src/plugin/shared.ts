...
export const DEPRECATED_PLUGIN_PACKAGES = ["agintes-openai-codex-auth", "agintes-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, agintesVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(agintesVersion) || semver.major(agintesVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.agintes
  if (typeof range !== "string") return
  if (!semver.satisfies(agintesVersion, range)) {
    throw new Error(`Plugin requires agintes ${range} but running ${agintesVersion}`)
  }
}
