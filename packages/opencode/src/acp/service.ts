...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@agintes-ai/sdk/v2"
...
export const AuthMethodID = "agintes-login"
...
export class Service extends Context.Service<Service, Interface>()("@agintes/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `agintes auth login` in the terminal",
      name: "Login with agintes",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, agintes provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const agintesProvider = providers[ProviderV2.ID.make("agintes")]
  const agintesModel = agintesProvider ? Provider.sort(Object.values(agintesProvider.models))[0] : undefined
  if (agintesProvider && agintesModel) return { providerID: agintesProvider.id, modelID: agintesModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Agintes service failure", service })
}
