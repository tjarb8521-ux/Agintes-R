...
import { AgintesHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for agintes serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AgintesHttpApi)
...
export const PublicApi = AgintesHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "agintes",
    version: "1.0.0",
    description: "agintes api",
