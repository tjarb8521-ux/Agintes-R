...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "agintes.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "agintes.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agintes.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "agintes.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["agintes.json", "agintes.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "agintes.json"),
        path.join(project, "agintes.jsonc"),
      ])
    })
  })
