...
// terminal sizes. The default agintes tui command is excluded —
// `agintes --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `agintes --help` prints today;
...
// top-level help on `--help` and exits 1; not a real agintes command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("agintes CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ agintes }) =>
      Effect.gen(function* () {
        const topLevel = yield* agintes.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* agintes.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`agintes ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`agintes ${argv.join(" ")} --help`)
        }
