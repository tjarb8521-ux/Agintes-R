...
describe("agintes acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ agintes }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ agintes }))
        ...
        expect(initialized.agentInfo?.name).toBe("Agintes")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ agintes }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ agintes })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("agintes-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "agintes-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ agintes }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("agintes-login")
