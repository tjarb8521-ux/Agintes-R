...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ agintes }) =>
    Effect.gen(function* () {
      const result = yield* agintes.spawn(["--replay-limit", "10"])

      agintes.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ agintes }) =>
    Effect.gen(function* () {
      const result = yield* agintes.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ agintes }) =>
    Effect.gen(function* () {
      const result = yield* agintes.spawn(["--mini", "--port", "4096"])
