...
import { PromptInput } from "@agintes-ai/schema/prompt-input"
import { Session } from "@agintes-ai/schema/session"
import { Project } from "@agintes-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@agintes-ai/schema/schema"
...
import { SessionEvent } from "@agintes-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Agintes process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Agintes process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
