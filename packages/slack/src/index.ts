...
import { createOpencode, type ToolPart } from "@agintes-ai/sdk"
...
console.log("🚀 Starting agintes server...")
const agintes = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await agintes.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new agintes session...")
    const { client, server } = agintes
    ...
    console.log("✅ Created agintes session:", createResult.data.id)
  ...
  console.log("📝 Sending to agintes:", message.text)
