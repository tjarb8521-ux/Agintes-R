...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("agintes-find")
  api.delete("agintes-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("agintes-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("agintes-find", new Highlight(...rest))
    return true
  }
