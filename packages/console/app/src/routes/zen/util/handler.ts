...
import { and, Database, eq, isNull, lt, or, sql } from "@agintes-ai/console-core/drizzle/index.js"
import { KeyTable } from "@agintes-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@agintes-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@agintes-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@agintes-ai/console-core/util/date.js"
import { Identifier } from "@agintes-ai/console-core/identifier.js"
import { Billing } from "@agintes-ai/console-core/billing.js"
import { Actor } from "@agintes-ai/console-core/actor.js"
import { WorkspaceTable } from "@agintes-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@agintes-ai/console-core/model.js"
import { Subscription } from "@agintes-ai/console-core/subscription.js"
import { BlackData } from "@agintes-ai/console-core/black.js"
import { UserTable } from "@agintes-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@agintes-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@agintes-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@agintes-ai/console-core/lite.js"
import { Resource } from "@agintes-ai/console-resource"
...
import { Workspace } from "@agintes-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-agintes-session") ?? ""
    const requestId = input.request.headers.get("x-agintes-request") ?? ""
    const ocClient = input.request.headers.get("x-agintes-client") ?? ""
    const projectId = input.request.headers.get("x-agintes-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://agintes.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-agintes-request")
            headers.delete("x-agintes-session")
            headers.delete("x-agintes-project")
            headers.delete("x-agintes-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-agintes-endpoint-id")
        const resEndpointModelId = res.headers.get("x-agintes-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://agintes.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://agintes.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://agintes.ai/workspace/${authInfo.workspaceID}/members`
