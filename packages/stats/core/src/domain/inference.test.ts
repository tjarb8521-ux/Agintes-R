...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve agintes route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "agintes")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "agintes")).toBe("openai")
    expect(statProvider("big-pickle", "", "agintes")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "agintes"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep agintes as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "agintes"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "agintes"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep agintes or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "agintes"), country: "US" })).toMatchObject([
