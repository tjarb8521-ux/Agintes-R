...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.agintes.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `agintes.settings.dat` is special: it maps to the `agintes.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"agintes.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // agintes.settings.dat → the electron settings store ("agintes.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "agintes.global.dat").
  const storeName = filename === "agintes.settings.dat" ? "agintes.settings" : filename
