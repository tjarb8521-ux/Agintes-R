...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Agintes launch", () => {
    expect(hasExistingAppState([file("agintes.settings")])).toBe(true)
    expect(hasExistingAppState([file("agintes.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("agintes")])).toBe(true)
  })
})
