...
export async function spawnWslSidecar(
  ...
  const agintes = await resolveWslOpencode(distro)
  if (!agintes) throw new Error(`Agintes is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "agintes"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    `exec ${shellEscape(agintes)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
