...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.agintes.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `agintes-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Agintes Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("agintes://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("agintes")
  registerRendererProtocol()
  setDockIcon()
