...
import type { UpdaterState } from "@agintes-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@agintes-ai/ui/logo"
...
const deepLinkEvent = "agintes:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `agintes.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://agintes.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("agintes.global.dat").getItem("language")
