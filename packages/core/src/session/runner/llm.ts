...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      // -- AGINTES R: Swarm Injection Hook --
      if (agent.id === "agintes-swarm") {
        // Find the last prompt from context to pass to the swarm
        const ctxMessages = yield* getContext(sessionID)
        const lastUserMsg = ctxMessages.slice().reverse().find(m => m.type === "user")
        let promptText = "Perform the next logical step."
        if (lastUserMsg && Array.isArray(lastUserMsg.content)) {
            const textPart = lastUserMsg.content.find((c: any) => c.type === "text")
            if (textPart && 'text' in textPart) promptText = textPart.text
        }
        
        yield* executeSwarm(sessionID, promptText)
