...
import type { IntegrationOAuthMethodRegistration } from "@agintes-ai/plugin/v2/effect/integration"
import { define } from "@agintes-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@agintes-ai/sdk/v2/types"
...
const defaultServer = "https://console.agintes.ai"
const clientID = "agintes-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("agintes")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Agintes provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("agintes", (integration) => {
        integration.name = "Agintes"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "agintes", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("agintes")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("agintes")
      ...
      const item = catalog.provider.get(ProviderV2.ID.agintes)
      if (!item) return
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("agintes")),
