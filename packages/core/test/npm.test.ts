...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@agintes/acme")).toBe("@agintes/acme")
    expect(Npm.sanitize("@agintes/acme@1.0.0")).toBe("@agintes/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/agintes/acme.git"
    const expected = win ? "acme@git+https_//github.com/agintes/acme.git" : spec
