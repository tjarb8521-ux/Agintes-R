...
import { GoogleVertexPlugin } from "@agintes-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.agintes, (provider) => {
          provider.api = {
            ...
            url: "https://agintes.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.agintes))
