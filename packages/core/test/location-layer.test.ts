...
import { define } from "@agintes-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@agintes-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "agintes.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "agintes.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
