...
import { Config } from "@agintes-ai/core/config"
import { ConfigProvider } from "@agintes-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@agintes-ai/core/v1/config/migrate"
...
import { Policy } from "@agintes-ai/core/policy"
import { Project } from "@agintes-ai/core/project"
...
describe("Config", () => {
  ...
  it.live("loads agintes JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "agintes.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "agintes.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "agintes.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "agintes.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "agintes.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "agintes.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .agintes configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".agintes"), { recursive: true })
            await fs.mkdir(path.join(directory, ".agintes"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "agintes.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "agintes.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "agintes.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "agintes.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "agintes.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".agintes", "agintes.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".agintes", "agintes.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".agintes")),
              AbsolutePath.make(path.join(directory, ".agintes")),
            ])
